	    ...
	    try {
			 ...
			 // Load the database driver
			  ...
			  out.println("<table border=1 >");  
	             out.println("<tr><th>name</th><th>previous month patients no.</th><tr>");  
	             while (rs.next()) {  
	                 String name = rs.getString("name"); 
	                 int previous_month_patients_no = rs.getInt("previous_month_patients_no"); 
	                 out.println("<tr><td>" + name + "</td><td>" + previous_month_patients_no + "</td>"
		                 		+ "<td><form action='center_details' method='POST'><input style='display:none' type='text' name='center' value="+ name +">"
